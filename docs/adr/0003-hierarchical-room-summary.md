...
- **v1 scope is the active JSONL chunk only — rotated chunks are forensic-only.** `RoomSummariser` reads from `activePath(room)`; once a chunk rotates at the 10k-line mark, content beyond the last summarised line in that chunk is no longer a candidate for new L1 generation. v2 deliverable (when real users hit the limitation): cross-chunk continuous line numbering so summarisation walks the entire transcript history including rotated chunks. v1 accepts the gap because the Nutshell covers strategic history regardless, and the active chunk is the high-signal recent-context layer.
