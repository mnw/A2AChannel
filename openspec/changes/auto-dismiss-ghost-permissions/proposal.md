...
- **`pty_await_pattern` Rust-side primitive** — generic regex-poll-with-timeout against an agent's tmux pane. Mirrors the shape of `terminal-control-mcp`'s `await_output`. Exposed as a Tauri command alongside `pty_tap_read` and `pty_capture_turn`. The ghost-permissions scraper is the first consumer; future features ("wait for build to finish", "detect when claude returns to idle prompt", etc.) reuse the same primitive.
- **Per-permission scraper watcher** wired into the existing `permission.new` broadcast. The watcher composes `pty_await_pattern` (positive-match) for the latch and `pty_await_pattern_absent` (inverse) for the debounce, instead of carrying its own snapshot loop. Keeps the state machine intent-level — "wait for dialog visible, then wait for it gone."
